...
class Face_Detector {
    ...
    public function __construct($detection_file = '/var/www/html/Lushtest/detection.dat') {
	...
	public function saveFace($file) {	
      $canvas = imagecreatetruecolor($this->face[‘w’], $this->face[‘w’]);
	imagecopy($canvas, $this->canvas, 0, 0, $this->face[‘x’], $this->face[‘x’], $this->face[‘w’], $this->face[‘w’]);
	//ob_start();
	imagejpeg($canvas,$file) or die ("Unable to put image in $file.");
	header('Content-type: image/jpeg');
        imagejpeg($canvas);
	
	//$data = ob_get_clean();
	//file_put_contents($file,$data) or die("Unable to put $canvas in $path.");	
		
	//imagedestroy($canvas);
    }
	public function avgColor(){
        $canvas = imagecreatetruecolor($this->face['w'], $this->face['w']);
        imagecopy($canvas, $this->canvas, 0, 0, $this->face['x'], $this->face['x'], $this->face['w'], $this->face['w']);
       // header('Content-type: image/jpeg');
       // imagejpeg($canvas);
